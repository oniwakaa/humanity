...
function GradientBackground({
  ...
  const generateGradient = () => {
    ...
    // Explicitly cast colors to avoid implicit any in map
    const typedColors = colors as { color: string; stop: string }[];
    const position = getGradientPosition(gradientOrigin);
    const colorStops = typedColors.map(({ color, stop }) => `${color} ${stop}`).join(',');
